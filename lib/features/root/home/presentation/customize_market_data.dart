...
class _CustomizeMarketDataState extends State<CustomizeMarketData> {
  ...
  String timeFrame = '60mins';

  @override
  Widget build(BuildContext context) {
    return BlocListener<AiInsightCubit, BaseState<MarketAnalysisData>>(
      listener: (_, state) {
            ...
            child: Column(
              ...
              children: [
                InputField.search(
                  onClick: () {
                    DialogHelper.show(context,
                        transparent: true, child: SearchAssetDialog());
                  },
                ),
                20.verticalSpace,
                Text(
                ...
                SelectableOptionComponent(
                    onSelect: (result) {
                      setState(() {
                        timeFrame = result.firstOrNull?.title ?? '60mins';
                      });
                    },
                    subtitle: 'Time Frame',
                    radio: true,
                    options: const [
                      SelectionEntity(title: '60mins'),
                      SelectionEntity(title: '4hrs'),
                      SelectionEntity(title: '1day'),
                      SelectionEntity(title: '30days'),
                    ]),
                20.verticalSpace,
                // SelectableOptionComponent(
                //     onSelect: (result) {},
                ...
                SelectableOptionComponent(
                    ...
                    options: const [
                      SelectionEntity(
                          title: 'Always show 3 assets on dashboard'),
                ...
                PrimaryButton.primary(
                  onPressed: () {
                    ...
                    context.read<AiInsightCubit>().generateAISpotLight(
                        symbols: watchList.map((e) => e.symbol ?? '').toList(),
                        timeframe: timeFrame);
                  },
                  text: 'Save Changes',
                ),
